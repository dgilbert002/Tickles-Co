...
async def run_prefilter(
    ...
    try:
        ...
        pass_to_strong = bool(parsed.get("pass_to_strong", False))
        is_chart = bool(parsed.get("is_chart", False))
        has_setup = bool(parsed.get("has_possible_setup", False))

        if not pass_to_strong:
            logger.info(
                "Pre-filter rejected image for %s: is_chart=%s has_setup=%s confidence=%s",
                instrument_symbol,
                is_chart,
                has_setup,
                parsed.get("confidence", 0),
            )
            reason = "not_a_chart" if not is_chart else ("no_setup" if not has_setup else "low_confidence")
            return LlmResult(
                direction="unclear",
                confidence=0.0,
...
async def fetch_pending_media(
    ...
    sql = (
        ...
        "   AND (tp.tracked_media_types IS NULL "
        "        OR tp.tracked_media_types NOT IN ('none', 'text')) "
...
class InterpretationService:
    ...
    async def _fetch_pending_text_news(self, shared_pool: DatabasePool) -> List[Dict[str, Any]]:
        ...
        sql = (
            ...
            " AND tp.is_tracked = TRUE "
            " AND (tp.tracked_media_types IS NULL "
            "      OR tp.tracked_media_types NOT IN ('none', 'media')) "
