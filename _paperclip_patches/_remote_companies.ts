...
import { companyProvisioningJobRoutes } from "./company-provisioning-jobs.js";
import {
  accessService,
  agentService,
  budgetService,
...
// Phase-3: fire-and-forget trigger to the Tickles MCP daemon. The executor
// there will POST progress back to Paperclip via the provisioning-jobs
// routes. Env: TICKLES_MCP_URL, TICKLES_MCP_TOKEN. Safe to leave unset — we
// log a warning and the company stays un-provisioned until company.provision
// is retried manually.
async function kickOffProvisioning(input: {
  companyId: string;
  jobId: string;
  templateId: string;
  slug: string;
  ...
  const url = process.env.TICKLES_MCP_URL ?? "http://127.0.0.1:7777/mcp";
  const token = process.env.TICKLES_MCP_TOKEN;
  const body = {
    jsonrpc: "2.0",
    id: `prov-${input.jobId}`,
    method: "tools/call",
    params: {
      ...
      // NOTE: MCP schema is camelCase (companyId, templateId, jobId). Keep
      // these keys in lockstep with shared/mcp/tools/provisioning.py or the
      // executor will silently skip job_id and the UI poller will stay on
      // "running" forever.
      arguments: {
        ...
        slug: input.slug,
        ruleOneMode: input.ruleOneMode,
        memuSubscriptions: input.memuSubscriptions,
      },
    },
  };
  ...
  try {
    ...
    console.warn("[companies] MCP company.provision call failed", {
      url,
      jobId: input.jobId,
...
export function companyRoutes(db: Db, storage?: StorageService) {
  ...
  router.use("/:companyId/provisioning-jobs", companyProvisioningJobRoutes(db));

  // UI-friendly alias: returns the same payload as
  // /provisioning-jobs/latest but with a stable "not_provisioned" fallback
  // instead of 404 so the wizard poller can render one code path.
  router.get("/:companyId/provisioning-status", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    const jobs = companyProvisioningJobService(db);
    const job = await jobs.latestByCompany(companyId);
    if (!job) {
      res.json({ status: "not_provisioned" as const });
      return;
    }
    res.json({ status: "job" as const, job });
  });
  ...
  router.post("/", validate(createCompanySchema), async (req, res) => {
    ...
    const { provisioning, ...createBody } = req.body ?? {};
    const company = await svc.create(createBody);
    ...
    // Phase-3: fire-and-forget provisioning if the wizard ticked the box.
    // The MCP daemon runs the 9-step executor and writes progress back to the
    // `company_provisioning_jobs` row we seeded here. Failures are logged but
    // never break company creation — the user can retry via company.provision.
    let provisioningJobId: string | null = null;
    if (provisioning?.enabled) {
      try {
        const jobsSvc = companyProvisioningJobService(db);
        const derivedSlug = company.name
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "_")
          .replace(/^_+|_+$/g, "")
          .slice(0, 32)
          .replace(/_+$/g, "");
        const slug = provisioning.slug ?? (derivedSlug.length > 0 ? derivedSlug : "company");
        const job = await jobsSvc.create(company.id, {
          templateId: provisioning.template ?? "blank",
          slug,
        });
        ...
        kickOffProvisioning({
          ...
          templateId: provisioning.template ?? "blank",
          slug,
          ruleOneMode: provisioning.ruleOneMode ?? "advisory",
          memuSubscriptions: provisioning.memuSubscriptions ?? [],
        }).catch((err) => {
          console.warn("[companies] provisioning kickoff failed", {
            companyId: company.id,
            jobId: job.id,
            error: err instanceof Error ? err.message : String(err),
          });
        ...
        console.warn("[companies] failed to seed provisioning job", {
          companyId: company.id,
          error: err instanceof Error ? err.message : String(err),
        });
      }
    ...
    res.status(201).json({ ...company, provisioningJobId });
  });
